...
def in_degree_distribution(digraph):
    '''
    Input: a dictionary representing a directed graph. Nodes are keys, edges are values in a set
    Output: a dictionary of nodes as keys with values equal to the integer sum of the edges pointing to the node
    dictionary naming convention trailing _d denotes dictionary
    '''
    if len(digraph) == 0:
        return {}
    ...
    histogram_data_d = {}
    for ele in digraph:
        histogram_data_d[ele] = len(digraph[ele])
...
print result_d
print in_degree_distribution(result_d)
#
